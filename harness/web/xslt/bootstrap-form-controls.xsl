<xsl:stylesheet version="1.0"
            ...
            <xsl:when test="chiba:data[@chiba:type='date' or @chiba:type='dateTime']">
                ...
                <xsl:choose>
                    <xsl:when test="chiba:data/@chiba:readonly='true'">
                        ...
                        <img id="{concat($id,'-', chiba:data/@chiba:type, '-button')}" src="images/calendar.gif" style="cursor:pointer;" class="disabled"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <input id="{concat($id,'-', chiba:data/@chiba:type, '-display')}" type="text" value="" title="{normalize-space(xforms:hint)}" readonly=""/>
                        <img id="{concat($id,'-', chiba:data/@chiba:type, '-button')}" src="images/calendar.gif" style="cursor:pointer;" class="enabled"/>
                    </xsl:otherwise>
                </xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:element name="input">
                    ...
                    <xsl:if test="$scripted='true'">
                        ...
                        <xsl:attribute name="onblur">javascript:setXFormsValue('
                            <xsl:value-of select="$id"/>');
                        </xsl:attribute>
                    </xsl:if>
                </xsl:element>
    ...
    <xsl:template name="image-trigger">
        <xsl:element name="input">
            ...
            <xsl:if test="$scripted='true'">
                <xsl:attribute name="onclick">javascript:activate('
                    <xsl:value-of select="$id"/>');
                </xsl:attribute>
            </xsl:if>
        </xsl:element>
                ...
                <xsl:element name="input">
                    ...
                    <xsl:if test="$scripted='true'">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:attribute name="onchange">javascript:upload('
                                    <xsl:value-of select="$id"/>');
                                </xsl:attribute>
                            </xsl:otherwise>
                        </xsl:choose>
    ...
    <xsl:template name="build-checkboxes">
        ...
        <xsl:for-each select="$items">
            ...
            <input id="{@id}-value" class="value" type="checkbox" name="{$name}" value="{xforms:value}" title="{$title}">
                ...
                <xsl:if test="$scripted='true'">
                    <xsl:attribute name="onclick">javascript:setXFormsValue('
